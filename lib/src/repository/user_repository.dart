...
import 'package:roundsman/src/repository/settings_repository.dart';
...
Future<User> updateDriverState(bool active) async{
  final String _apiToken = 'api_token=${currentUser.value.apiToken}';
  final String url = '${GlobalConfiguration().getString(
      'api_base_url')}driver/status?$_apiToken';
  final client = new http.Client();
  ...
  if(response.statusCode == 200 || response.statusCode == 204) {
    currentUser.value.active = active;
    SharedPreferences prefs = await SharedPreferences.getInstance();
    User user = await currentUser.value;
    String jsonUser = jsonEncode(user.toMap());
    await prefs.setString('current_user', jsonUser);
    print(prefs);
  }
  return currentUser.value;
}
...
Future<String> uploadImage(File file) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  Dio dio = new Dio();
  dio.options.baseUrl = '${GlobalConfiguration().getString('api_base_url')}users/upload-avatar?$_apiToken';
