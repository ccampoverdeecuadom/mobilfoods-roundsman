...
class _ProfileWidgetState extends StateMVC<ProfileWidget> {
  ...
  void getImage() async {
    ...
    if (pickedFile != null) {
      setState(() => _con.isUploadingImage = true);
      _image = File(pickedFile.path);
      _con.updateProfileImage(_image);
    } else {
  ...
  void initState() {
    _con.listenForRecentOrders();
      _con.imageUri = currentUser.value.image?.url ?? '';
      _con.isUploadingImage = false;
    super.initState();
  }
...
//              padding: const EdgeInsets.symmetric(horizontal: 0, vertical: 10),
              child: Column(
                children: <Widget>[
                  ProfileAvatarWidget(user: currentUser.value, dataUpdated: getImage, isUploadingImage: _con.isUploadingImage, imageUrl: _con.imageUri,),
                  ListTile(
